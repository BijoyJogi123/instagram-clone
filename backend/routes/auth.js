...
auth.post('/signup',[
    ...
    body('full_name','dont use only charecter').isLength({min:4,max:14}),
], async (req,res)=>{
    let  success = false;
    ...
    try {
     ...
     res.send({success,authToken,data2});  //here is user id
...
auth.post('/login',[
    ...
    body('password','please enter a password').isLength({ min: 8}).exists(), 
],async(req,res)=>{
...
auth.post('/getalluser',verifyToken,async(req,res)=>{
